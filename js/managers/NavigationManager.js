...
class NavigationManager {
  ...
  getOrCreateView(viewId, container) {
    ...
    if (!ViewClass) {
      console.error(
        `NavigationManager: Class '${config.className}' not found for view '${viewId}'`
      );
      return null;
    }
  ...
  navigateTo(viewId, params = {}, isPopState = false) {
    ...
    if (this.header) {
      const systemName = SystemService.getCurrentSystem()
        ? SystemService.getCurrentSystem().systemName
        : 'System';
