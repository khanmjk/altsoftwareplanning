...
class SystemRepository {
  constructor(options = {}) {
    ...
    this.previousStorageKey = PREVIOUS_SYSTEMS_KEY;
    ...
    this.settingsDriver =
      options.settingsDriver ||
      this._createDriver(this.settingsStorageMode, this.settingsStorageKey);

    this.prefsStorageMode = options.prefsStorageMode || 'local';
    this.prefsStorageKey = options.prefsStorageKey || DEFAULT_UI_PREFS_KEY;
    this.prefsDriver =
      options.prefsDriver || this._createDriver(this.prefsStorageMode, this.prefsStorageKey);

    // Auto-migrate from previous version if needed
    this._migrateFromPreviousVersion();
  }
  ...
  configure(options = {}) {
    ...
    const nextDriver =
      options.driver ||
      (hasStorageKeyChange || hasStorageModeChange
        ? this._createDriver(nextStorageMode, nextStorageKey)
        : this.driver);
    ...
    const nextSettingsDriver =
      options.settingsDriver ||
      (hasSettingsKeyChange || hasSettingsModeChange
        ? this._createDriver(nextSettingsMode, nextSettingsKey)
        : this.settingsDriver);
    ...
    const nextPrefsDriver =
      options.prefsDriver ||
      (hasPrefsKeyChange || hasPrefsModeChange
        ? this._createDriver(nextPrefsMode, nextPrefsKey)
        : this.prefsDriver);
