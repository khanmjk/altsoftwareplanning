...
class InitiativeEditComponent {
  ...
  render() {
    ...
    const visibleInitiatives = this.systemData.yearlyInitiatives
      .map((init, index) => ({ init, index }))
      .filter(({ init }) => this._matchesSearch(init));

    const hasItems = visibleInitiatives.length > 0;
    ...
    visibleInitiatives.forEach(({ init, index }) => {
      if (!init) return;
  ...
  _createInitiativeItem(init, index) {
    ...
    details.appendChild(
      this._createFormGroup('Description', 'textarea', 'description', init.description, index)
    );
    ...
    grid1.appendChild(
      this._createSelectGroup('Status', 'status', init.status, statusOptions, index)
    );

    grid1.appendChild(
      this._createFormGroup('Target Due Date', 'date', 'targetDueDate', init.targetDueDate, index)
    );
    details.appendChild(grid1);

    // 2. Strategic Alignment
    // Themes (Multi-Select using ThemedSelect)
    const themesOptions = (this.systemData.definedThemes || []).map((t) => ({
      value: t.themeId,
      text: t.name,
    }));
    details.appendChild(
      this._createMultiSelectGroup('Themes', 'themes', init.themes || [], themesOptions, index)
    );

    const goalsOptions = [
      ...(this.systemData.goals || []),
      ...(this.systemData.subGoals || []),
    ...
    const currentPmVal = init.projectManager
      ? `${init.projectManager.type}:${init.projectManager.id}`
      : '';
    grid2.appendChild(
    ...
    details.appendChild(
      this._createCollapsibleSection(
        'Team Assignments',
        this._createAssignmentsContent(init, index)
      )
    );

    // 5. ROI (Collapsible)
    details.appendChild(
      this._createCollapsibleSection('ROI Details', this._createROIContent(init, index))
    );
  ...
  _matchesSearch(init) {
    ...
    const themeMap = new Map((this.systemData.definedThemes || []).map((t) => [t.themeId, t.name]));
    const teamMap = new Map(
      (this.systemData.teams || []).map((t) => [t.teamId, t.teamIdentity || t.teamName])
    );

    const themeNames = (init.themes || [])
      .map((themeId) => themeMap.get(themeId) || themeId)
      .join(' ');
    const teamNames = (init.assignments || [])
      .map((a) => teamMap.get(a.teamId) || a.teamId)
      .join(' ');
  ...
  _createROIContent(init, index) {
    ...
    grid.appendChild(
      this._createSelectGroup('Category', 'roi.category', roi.category, categories, index, true)
    );

    // Value Type
    const valueTypes = ['Hard Savings', 'Soft Savings', 'Revenue', 'QualitativeScore', 'Narrative'];
    grid.appendChild(
      this._createSelectGroup('Value Type', 'roi.valueType', roi.valueType, valueTypes, index, true)
    );
    ...
    grid.appendChild(
      this._createSelectGroup('Currency', 'roi.currency', roi.currency, currencies, index, true)
    );
    ...
    if (index === 'draft') {
      ...
      if (success) {
        ...
        notificationManager.showToast(
          `Initiative "${init.title}" created successfully.`,
          'success'
        );
        this.render();
      } else {
      ...
      if (success) {
        notificationManager.showToast(
          `Initiative "${init.title}" updated successfully.`,
          'success'
        );
