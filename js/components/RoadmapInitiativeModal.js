...
class RoadmapInitiativeModal {
  ...
  render() {
    ...
    // Description
    form.appendChild(
      this._createFormGroup('Description', 'textarea', 'description', null, false, 3)
    );
    ...
    // Status Select
    grid1.appendChild(
      this._createThemedSelect('Status', 'status', 'roadmapModalStatusSelect', [
    ...
    grid1.appendChild(
      this._createFormGroup('Target Due Date *', 'date', 'targetDueDate', null, true)
    );
    form.appendChild(grid1);

    // PM Notes
    const notesGroup = this._createFormGroup(
      'PM Capacity/Team Notes',
      'textarea',
      'pmNotes',
      null,
      false,
    ...
    // Themes (Multi-Select)
    form.appendChild(
      this._createThemedSelect('Themes', 'themes', 'roadmapModalThemesSelect', [], true)
    );
    ...
    grid2.appendChild(
      this._createThemedSelect('Primary Goal', 'primaryGoalId', 'roadmapModalGoalSelect')
    );
    ...
    // Project Manager
    form.appendChild(
      this._createThemedSelect(
        'Project Manager',
        'projectManager',
        'roadmapModalProjectManagerSelect'
      )
    );
    ...
    const horizonGroup = this._createFormGroup(
      'Time Horizon (Months)',
      'number',
      'roiTimeHorizon',
      null,
    ...
    const confGroup = this._createFormGroup(
      'Confidence Level',
      'text',
      'roiConfidenceLevel',
      null,
    ...
    form.appendChild(
      this._createFormGroup(
        ...
        'textarea',
        'roiCalculationMethodology',
        null,
        false,
        2
      )
    );
    const bcLinkGroup = this._createFormGroup(
  ...
  _createThemedSelect(labelText, selectName, elementId, initialOptions = [], isMultiple = false) {
    ...
    const selectOptions =
      initialOptions.length > 0 ? initialOptions : [{ value: '', text: 'Loading...' }];
  ...
  populateDropdowns() {
    ...
    const owners = [
      ...(SystemService.getCurrentSystem().sdms || []).map((p) => ({
        value: `sdm:${p.sdmId}`,
        text: `${p.sdmName} (SDM)`,
      })),
      ...(SystemService.getCurrentSystem().pmts || []).map((p) => ({
        value: `pmt:${p.pmtId}`,
        text: `${p.pmtName} (PMT)`,
      })),
      ...(SystemService.getCurrentSystem().seniorManagers || []).map((p) => ({
  ...
  loadInitiativeData(initiativeId) {
    ...
    if (initiative.projectManager) {
      this.selects
        .get('projectManager')
        .setValue(`${initiative.projectManager.type}:${initiative.projectManager.id}`);
    } else {
    ...
    if (initiative.roi) {
      ...
      form.elements['roiCalculationMethodology'].value =
        initiative.roi.calculationMethodology || '';
    ...
    // Assignments
    this.assignments = initiative.assignments
      ? JSON.parse(JSON.stringify(initiative.assignments))
      : [];
    this.renderAssignments();
  }
