...
class ImpactWidget {
  ...
  render() {
    ...
    this.svg = D3Service.select(svgContainer)
      .append('svg')
      .attr('width', '100%')
      .attr('height', '800px');
  ...
  updateDynamicSelector() {
    ...
    switch (mode) {
      case 'initiative':
        items = (systemData.yearlyInitiatives || []).sort((a, b) => a.title.localeCompare(b.title));
      ...
      case 'team':
        items = (systemData.teams || []).sort((a, b) =>
          (a.teamIdentity || a.teamName).localeCompare(b.teamIdentity || b.teamName)
        );
      ...
      case 'service':
        items = (systemData.services || []).sort((a, b) =>
  ...
  generateInitiativeGraph(selectedInitiativeId) {
    ...
    const initiative = systemData.yearlyInitiatives.find(
      (i) => i.initiativeId === selectedInitiativeId
    );
    ...
    (initiative.assignments || []).forEach((assignment) => {
      ...
      if (team) {
        addNode({
          id: team.teamId,
          name: team.teamIdentity || team.teamName,
          type: 'Team',
          data: team,
        });
        links.push({
          source: initiative.initiativeId,
          target: team.teamId,
          type: 'assignment',
          sde: assignment.sdeYears,
        });
      }
    });
  ...
  generateTeamGraph(selectedTeamId) {
    ...
    const assignedInitiatives = systemData.yearlyInitiatives.filter((init) =>
      (init.assignments || []).some((a) => a.teamId === selectedTeamId)
    );
    ...
    assignedInitiatives.forEach((initiative) => {
      ...
      links.push({
        source: team.teamId,
        target: initiative.initiativeId,
        type: 'assignment',
        sde: assignment.sdeYears,
      });
      ...
      (initiative.assignments || []).forEach((ass) => {
        if (ass.teamId !== selectedTeamId) {
          ...
          if (otherTeam) {
            addNode({
              id: otherTeam.teamId,
              name: otherTeam.teamIdentity || otherTeam.teamName,
              type: 'Team',
              data: otherTeam,
            });
            links.push({
              source: initiative.initiativeId,
              target: otherTeam.teamId,
              type: 'assignment',
              sde: ass.sdeYears,
            });
          }
  ...
  generateServiceGraph(selectedServiceName) {
    ...
    const impactingInitiatives = systemData.yearlyInitiatives.filter((init) =>
      (init.impactedServiceIds || []).includes(selectedServiceName)
    );
          ...
          .radius(
            (d) =>
              (d.type === 'Team'
                ...
                : d.type === 'Initiative'
                  ? 28
                  : (serviceImpactCount.get(d.id) || 0) * 5 + 20) + 10
          )
      );
    ...
    nodeGroup
      ...
      .attr('r', (d) =>
        d.type === 'Team'
          ...
          : d.type === 'Initiative'
            ? 28
            : (serviceImpactCount.get(d.id) || 0) * 4 + 18
      )
      .attr('fill', (d) => {
  ...
  updateSummary(nodes, selectedId) {
    ...
    if (selectedNode) {
      ...
      const teamsImpacted = (init.assignments || [])
        .map((a) => {
  ...
  renderLegend(nodes, teamColors, initiativeStatusColors) {
    ...
    const uniqueStatuses = [
      ...new Set(nodes.filter((n) => n.type === 'Initiative').map((n) => n.data.status)),
    ].sort();
    if (uniqueStatuses.length > 0) {
      ...
      uniqueStatuses.forEach((status) => {
        legendData.push({
          label: status || 'N/A',
          color: initiativeStatusColors[status] || '#ff7f0e',
          type: 'item',
        });
      });
    }
