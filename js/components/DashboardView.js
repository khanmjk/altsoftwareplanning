...
class DashboardView {
  constructor(containerId) {
    ...
    this.widgets = [
      {
        id: 'strategicGoalsWidget',
        title: 'Strategic Goals Dashboard',
        label: 'Strategic Goals',
        icon: 'fas fa-bullseye',
        generator: () => this.initializeGoalsWidget(),
      },
      {
        id: 'accomplishmentsWidget',
        title: 'Accomplishments',
        label: 'Accomplishments',
        icon: 'fas fa-trophy',
        generator: () => this.initializeAccomplishmentsWidget(),
      },
      {
        id: 'investmentDistributionWidget',
        title: 'Investment Distribution by Theme',
        label: 'Investment Distribution',
        icon: 'fas fa-chart-pie',
        generator: () => this.generateInvestmentDistributionChart(),
      },
      {
        id: 'investmentTrendWidget',
        title: 'Investment Trend Over Time',
        label: 'Investment Trend',
        icon: 'fas fa-chart-line',
        generator: () => this.generateInvestmentTrendChart(),
      },
      {
        id: 'teamDemandWidget',
        title: 'Team Demand by Quarter',
        label: 'Team Demand',
        icon: 'fas fa-users-cog',
        generator: () => this.initializeTeamDemandWidget(),
      },
      {
        id: 'roadmapTimelineWidget',
        title: 'Roadmap by Quarter',
        label: 'Roadmap Timeline',
        icon: 'fas fa-calendar-alt',
        generator: () => this.initializeRoadmapTableWidget(),
      },
      {
        id: 'threeYearPlanWidget',
        title: '3-Year Plan (3YP)',
        label: '3-Year Plan',
        icon: 'fas fa-sitemap',
        generator: () => this.initialize3YPWidget(),
      },
      {
        id: 'initiativeImpactWidget',
        title: 'Initiative Impact',
        label: 'Initiative Impact',
        icon: 'fas fa-chart-bar',
        generator: () => this.initializeImpactWidget(),
      },
    ];
  }
  ...
  _getDefaultPlanningYear() {
    ...
    const years = [
      ...new Set(
        system.yearlyInitiatives.map((init) => init.attributes?.planningYear).filter(Boolean)
      ),
    ].sort((a, b) => a - b);
  ...
  navigateWidget(direction) {
    const newIndex =
      (this.currentWidgetIndex + direction + this.widgets.length) % this.widgets.length;
          ...
          tooltip: {
            callbacks: {
              label: (context) => {
                ...
                if (context.raw !== null) {
                  const percentage =
                    data.total > 0 ? ((context.raw / data.total) * 100).toFixed(1) : 0;
    ...
    const allYears = [
      ...new Set(
        SystemService.getCurrentSystem()
          .yearlyInitiatives.map((init) => init.attributes.planningYear)
          .filter(Boolean)
      ),
    ].sort((a, b) => a - b);

    const allThemes = [
      ...new Set(SystemService.getCurrentSystem().definedThemes.map((t) => t.name)),
    ];
  ...
  renderTeamDemandChart() {
    ...
    const teamColorScale = d3
      .scaleOrdinal(d3.schemeCategory10)
      .domain(SystemService.getCurrentSystem().teams.map((t) => t.teamId));
  ...
  processInvestmentData(selectedYear) {
    const themeMap = new Map(
      SystemService.getCurrentSystem().definedThemes.map((theme) => [theme.themeId, theme.name])
    );
    const investmentByTheme = {};

    themeMap.forEach((name) => {
    ...
    const initiatives =
      selectedYear === 'all'
        ? SystemService.getCurrentSystem().yearlyInitiatives
        : SystemService.getCurrentSystem().yearlyInitiatives.filter(
            (init) => init.attributes.planningYear == selectedYear
          );

    initiatives.forEach((initiative) => {
      if (initiative.status === 'Completed') return;
      const totalSdeYears = (initiative.assignments || []).reduce(
        (sum, a) => sum + (a.sdeYears || 0),
        0
      );
      if (totalSdeYears > 0) {
