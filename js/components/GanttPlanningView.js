...
class GanttPlanningView {
  constructor(containerId) {
    ...
    this.ganttStatusFilter = new Set([
      'Backlog',
      'Defined',
      'Committed',
      'In Progress',
    ...
    const storedGanttWidth = parseFloat(
      SettingsService.getUiPreference(this.GANTT_TABLE_WIDTH_KEY, null)
    );
  ...
  setupScrollSync() {
    ...
    const frappeScrollable =
      chartWrapper.querySelector('.gantt-container') ||
      chartWrapper.querySelector('.gantt') ||
      chartWrapper;

    // Also get the table's scrollable wrapper if it exists
    const tableScrollable = tableContainer.querySelector('.gantt-table-wrapper') || tableContainer;
  ...
  generateGanttToolbar() {
    ...
    const initialValue = validCurrentFilter.length > 0 ? validCurrentFilter : uniqueStatuses;
  ...
  getGanttFilteredInitiatives() {
    ...
    if (this.tableController && this.tableController.model) {
      ...
      return GanttService.getFilteredInitiatives({
        ...
        groupValue:
          filters.groupValue || document.getElementById('ganttGroupValue')?.value || 'all',
    ...
    if (yearFilter && yearFilter !== 'all') {
      initiatives = initiatives.filter(
        (init) => (init.attributes?.planningYear || '').toString() === yearFilter.toString()
      );
    }
  ...
  async renderGanttChart() {
    ...
    const selectedTeam =
      this.currentGanttGroupBy === 'Team'
        ? document.getElementById('ganttGroupValue')?.value || 'all'
        : null;
  ...
  getGanttFocusContext() {
    return {
      ...
      initiativeId: this.lastGanttFocusInitiativeId
        ? this.normalizeGanttId(this.lastGanttFocusInitiativeId)
        : null,
    };
  }
  ...
  handleGanttToggleFromChart(task) {
    ...
    if (task.type === 'initiative') {
      ...
      if (this.ganttExpandedInitiatives.has(id)) {
        ...
        this.ganttExpandedInitiatives.clear();
        const hasWPs = (SystemService.getCurrentSystem().workPackages || []).some(
          (wp) => wp.initiativeId === id
        );
        if (hasWPs) {
  ...
  scrollToTableFocusRow(task) {
    ...
    setTimeout(() => {
      ...
      if (task.type === 'initiative' && task.initiativeId) {
        ...
        focusRow = tableContainer.querySelector(
          `tr[data-wp-id="${task.workPackageId}"][data-team-id="${task.teamId}"]`
        );
      }
  ...
  handleGanttUpdate({ task, start, end }) {
    ...
    const hasWPs = (id) =>
      (SystemService.getCurrentSystem().workPackages || []).some((wp) => wp.initiativeId === id);
    ...
    if (task.type === 'initiative') {
      ...
      const wpId = task.workPackageId;
      const wp = (SystemService.getCurrentSystem().workPackages || []).find(
        (w) => w.workPackageId === wpId
      );
      if (!wp) return;
  ...
  scrollToGanttFocusTask() {
    ...
    const container = document.getElementById('ganttChartContainer');
    if (!container) return;
    const target = Array.from(container.querySelectorAll('.bar-wrapper')).find(
      (el) => this.normalizeGanttId(el.getAttribute('data-id')) === focusId
    );
    if (target) {
