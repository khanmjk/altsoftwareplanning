...
class RoadmapComponent {
  ...
  renderFilters() {
    ...
    // 5. Goal Filter
    const goalOptions = [{ value: 'all', text: 'All Goals' }];
    (SystemService.getCurrentSystem().goals || []).forEach((goal) => {
      goalOptions.push({
        value: goal.goalId,
        text: goal.name || goal.title || goal.goalId,
      });
    });

    const goalWrap = this._createFilterWrapper('Goal:');
    this._goalSelect = new ThemedSelect({
      options: goalOptions,
      value: 'all',
      id: 'roadmapGoalFilter',
      onChange: () => this.renderGrid(),
    });
    goalWrap.appendChild(this._goalSelect.render());
    filtersContainer.appendChild(goalWrap);

    // 6. Status Filter
    const statusOptions = [{ value: 'all', text: 'All Statuses' }];
    const statusSet = new Set();
    (SystemService.getCurrentSystem().yearlyInitiatives || []).forEach((init) => {
      const status = (init.status || '').trim();
      if (status) statusSet.add(status);
    });
    const sortedStatuses = Array.from(statusSet).sort((a, b) => a.localeCompare(b));
    sortedStatuses.forEach((status) => {
      statusOptions.push({ value: status, text: status });
    });

    const statusWrap = this._createFilterWrapper('Status:');
    this._statusSelect = new ThemedSelect({
      options: statusOptions,
      value: 'all',
      id: 'roadmapStatusFilter',
      onChange: () => this.renderGrid(),
    });
    statusWrap.appendChild(this._statusSelect.render());
    filtersContainer.appendChild(statusWrap);
  ...
  prepareData() {
    ...
    const goalFilter = this._goalSelect?.getValue() || 'all';
    const statusFilter = this._statusSelect?.getValue() || 'all';
    const selectedStatuses = statusFilter !== 'all' ? [statusFilter] : [];
  ...
  _createRoadmapCard(init) {
    ...
    const sdeTotal = (init.assignments || [])
      .reduce((sum, a) => sum + (a.sdeYears || 0), 0)
      .toFixed(1);
    const themeNames =
      (init.themes || [])
        .map((tid) => {
  ...
  handleDrop(event, target) {
    ...
    document
      .querySelectorAll('.roadmap-quarter-cell')
  ...
  updateInitiative(initiativeId, target) {
    const initiativeIndex = SystemService.getCurrentSystem().yearlyInitiatives.findIndex(
    ...
    if (this.viewType === 'quarterly') {
      ...
      const oldQuarter =
        ...
        RoadmapService.getQuarterFromDate(initiative.targetDueDate) ||
        'Backlog';

      updates.targetQuarter = newQuarter;

      const year =
        this.planningYear && this.planningYear !== 'all'
    ...
    const success = InitiativeService.updateInitiative(
