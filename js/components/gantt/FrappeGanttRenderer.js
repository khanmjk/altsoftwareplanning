...
class FrappeGanttRenderer extends GanttRenderer {
  ...
  _resolveFocusContext(taskIdOrContext) {
    ...
    if (typeof taskIdOrContext === 'object') {
      ...
      return {
        ...
        initiativeId:
          taskIdOrContext.initiativeId ||
          task?.initiativeId ||
          (task?.type === 'initiative' ? task?.id : null),
      };
    }
  ...
  _applyFocusStyles(wrapper, tasks, focus = {}) {
    ...
    bars.forEach((bar) => {
      ...
      const isInitFocus =
        focusType === 'initiative' && focusInitiativeId && idNorm === focusInitiativeId;
  ...
  _populateArrowLabels(wrapper, arrowEl) {
    if (!arrowEl) return;
    const fromId = this._normalizeId(
      arrowEl.dataset.fromId || arrowEl.dataset.from || arrowEl.getAttribute('data-from')
    );
    const toId = this._normalizeId(
      arrowEl.dataset.toId || arrowEl.dataset.to || arrowEl.getAttribute('data-to')
    );
  ...
  _lookupTaskLabel(wrapper, normId) {
    ...
    const match = Array.from(wrapper.querySelectorAll('.bar-wrapper[data-id]')).find(
      (bw) => this._normalizeId(bw.getAttribute('data-id')) === id
    );
    if (match) {
  ...
  _formatDependencyText(wrapper, arrowEl) {
    ...
    const depKey =
      toId && fromId ? `${this._normalizeId(toId)}|${this._normalizeId(fromId)}` : null;
    if (depKey && this._dependencyTextMap && this._dependencyTextMap.has(depKey)) {
      const txt = this._dependencyTextMap.get(depKey);
      return txt;
    }

    const fromLabel = fromLabelDs || this._lookupTaskLabel(wrapper, fromId) || null;
    const toLabel = toLabelDs || this._lookupTaskLabel(wrapper, toId) || null;
