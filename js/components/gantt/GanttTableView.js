...
class GanttTableView {
  ...
  _renderRows(normalizedData, model, context) {
    ...
    const { allInitiatives = [], filteredInitiatives = [], teams = [] } = context;
    ...
    normalizedData.forEach((init) => {
      ...
      const isFocusRow =
        focus.taskType === 'initiative' && focus.taskId && initIdNorm === focus.taskId;
  ...
  _renderWorkPackages(init, model, context, isFocusInitiative) {
    ...
    wps.forEach((wp) => {
      ...
      const isFocusWp =
        focus.taskType === 'workPackage' && focus.taskId && wpIdNorm === focus.taskId;
