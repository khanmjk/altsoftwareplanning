...
class TeamEditComponent {
  ...
  _createTeamItem(team, index) {
    ...
    header.onclick = () => {
      ...
      if (isExpanded) {
        ...
        // Collapse others
        document
          .querySelectorAll('.team-edit-details')
    ...
    // Basic Info
    details.appendChild(
      this._createFormGroup('Team Identity:', 'input', 'teamIdentity', team.teamIdentity, index)
    );
    details.appendChild(
      this._createFormGroup('Team Name:', 'input', 'teamName', team.teamName, index)
    );
    details.appendChild(
    ...
    details.appendChild(
      this._createCollapsibleSection(
        'Services Owned',
        this._createServicesOwnedContent(team, index)
      )
    );
    ...
    details.appendChild(
      this._createCollapsibleSection(
        'SDM Assignment',
        this._createSdmAssignmentContent(team, index)
      )
    );
    ...
    details.appendChild(
      this._createCollapsibleSection(
        'PMT Assignment',
        this._createPmtAssignmentContent(team, index)
      )
    );
    ...
    details.appendChild(
      this._createCollapsibleSection(
        'Team Engineer Assignment',
        this._createEngineersContent(team, index)
      )
    );

    // Away Team (Collapsible)
    details.appendChild(
      this._createCollapsibleSection('Away-Team Members', this._createAwayTeamContent(team, index))
    );
      ...
      if (!readOnly && this.systemData.teams[index]) {
        ...
        if (fieldName === 'teamIdentity' || fieldName === 'teamName') {
          const headerTitle = e.target
            .closest('.team-edit-item')
            .querySelector('.team-edit-header span:last-child');
          const t = this.systemData.teams[index];
          if (headerTitle)
            headerTitle.innerText = `Team: ${t.teamIdentity || t.teamName || 'New Team'}`;
        }
      }
    });
  ...
  _createServicesOwnedContent(team, index) {
    ...
    const allServices = (this.systemData.services || []).map((service) => ({
      value: service.serviceName,
      text: service.serviceName,
      owningTeamId: service.owningTeamId || null,
    }));
    const currentServicesForTeam =
      (team.teamId && allServices.filter((s) => s.owningTeamId === team.teamId)) || [];
    const availableServicesForTeam = allServices.filter(
      (s) => !s.owningTeamId || s.owningTeamId === null
    );
    ...
    const container = new DualListSelector({
      ...
      moveCallback: (movedServiceValue, direction, currentTeamIndexCallback) => {
        const serviceToUpdate = this.systemData.services.find(
          (s) => s.serviceName === movedServiceValue
        );
  ...
  _createSdmAssignmentContent(team, index) {
    ...
    const sdmContainer = this._createReportingStructureDualList(team, index, sdmSection);
  ...
  /**
   * Creates a unified reporting structure selector that includes both SDMs and Senior Managers.
   * This enables flexible hierarchy where teams can report to either:
   * - An SDM (standard path)
   * - Directly to a Senior Manager (bypassing SDM)
   */
  _createReportingStructureDualList(team, index, sdmSection) {
    const allSdms = this.systemData.sdms || [];
    const allSeniorManagers = this.systemData.seniorManagers || [];

    // Determine current manager (could be SDM or Senior Manager)
    const currentSdm = allSdms.find((sdm) => sdm && sdm.sdmId === team.sdmId);
    const currentSrMgr =
      !team.sdmId && team.seniorManagerId
        ? allSeniorManagers.find((sm) => sm && sm.seniorManagerId === team.seniorManagerId)
        : null;

    // Build current option
    let currentOptions = [];
    if (currentSdm) {
      currentOptions = [{ value: `sdm:${currentSdm.sdmId}`, text: `${currentSdm.sdmName} (SDM)` }];
    } else if (currentSrMgr) {
      currentOptions = [
        {
          value: `srMgr:${currentSrMgr.seniorManagerId}`,
          text: `${currentSrMgr.seniorManagerName} (Sr Manager - Direct)`,
        },
      ];
    }

    // Build available options - SDMs first, then Senior Managers with a visual separator
    const availableSdmOptions = allSdms
      .filter((sdm) => sdm && (!team.sdmId || sdm.sdmId !== team.sdmId))
      .map((s) => ({ value: `sdm:${s.sdmId}`, text: `${s.sdmName} (SDM)` }));

    const availableSrMgrOptions = allSeniorManagers
      .filter((sm) => sm && (!team.seniorManagerId || sm.seniorManagerId !== team.seniorManagerId))
      .map((s) => ({
        value: `srMgr:${s.seniorManagerId}`,
        text: `${s.seniorManagerName} (Sr Manager - Direct)`,
      }));

    // Combine with SDMs first, then Senior Managers
    const availableOptions = [...availableSdmOptions, ...availableSrMgrOptions];

    return new DualListSelector({
      contextIndex: index,
      leftLabel: 'Reports To:',
      rightLabel: 'Available Managers (SDMs & Sr Managers):',
      currentOptions: currentOptions,
      availableOptions: availableOptions,
      leftField: 'currentManager',
      rightField: 'availableManagers',
      moveCallback: (movedValue, directionCallback, teamIndexCallback) => {
        const targetTeam = this.systemData.teams[teamIndexCallback];
        if (!targetTeam) return;

        if (directionCallback === 'add') {
          // Parse the value to determine type: "sdm:sdmId" or "srMgr:seniorManagerId"
          const [type, id] = movedValue.split(':');
          if (type === 'sdm') {
            targetTeam.sdmId = id;
            targetTeam.seniorManagerId = null;
          } else if (type === 'srMgr') {
            targetTeam.sdmId = null;
            targetTeam.seniorManagerId = id;
          }
        } else {
          // Remove - clear both
          targetTeam.sdmId = null;
      ...
      addNewCallback: (newSdmNameInput) => {
        ...
        if (
          (this.systemData.sdms || []).some(
            (s) => s && s.sdmName.toLowerCase() === newSdmNameInput.toLowerCase()
          )
        ) {
          notificationManager.showToast(`SDM "${newSdmNameInput}" already exists.`, 'warning');
          return { preventAdd: true };
        }
        const newSdmObject = {
          sdmId: 'sdm-' + Date.now(),
          sdmName: newSdmNameInput,
          seniorManagerId: null,
        };
        if (!this.systemData.sdms) this.systemData.sdms = [];
        this.systemData.sdms.push(newSdmObject);
        return { value: `sdm:${newSdmObject.sdmId}`, text: `${newSdmObject.sdmName} (SDM)` };
      },
    }).render();
  }
  ...
  _createPmtDualList(team, index) {
    ...
    return new DualListSelector({
      ...
      availableOptions: allPmts
        .filter((pmt) => pmt && (!team.pmtId || pmt.pmtId !== team.pmtId))
        .map((p) => ({ value: p.pmtId, text: p.pmtName })),
      ...
      addNewCallback: (newPmtNameInput) => {
        ...
        if (
          (this.systemData.pmts || []).some(
            (p) => p && p.pmtName.toLowerCase() === newPmtNameInput.toLowerCase()
          )
        ) {
          notificationManager.showToast(`PMT "${newPmtNameInput}" already exists.`, 'warning');
          return { preventAdd: true };
        }
  ...
  _createEngineersDualList(team, index) {
    const currentEngineerOptions = (team.engineers || []).map((engineerName) => {
      const engDetails = (this.systemData.allKnownEngineers || []).find(
    ...
    const availableEngineerOptions = (this.systemData.allKnownEngineers || [])
      ...
      .map((knownEng) => {
        ...
        if (knownEng.currentTeamId) {
          const assignedTeam = (this.systemData.teams || []).find(
            (t) => t.teamId === knownEng.currentTeamId
          );
          teamContext = assignedTeam
            ? assignedTeam.teamIdentity || assignedTeam.teamName
            : `Assigned Elsewhere (${knownEng.currentTeamId.slice(-4)})`;
        }
        return {
          value: knownEng.name,
    ...
    const container = new DualListSelector({
      ...
      moveCallback: (movedEngineerName, direction, currentTeamEditIndex) => {
        ...
        const engineerGlobal = (this.systemData.allKnownEngineers || []).find(
          (ke) => ke.name === movedEngineerName
        );
        ...
        if (direction === 'add') {
          if (oldTeamIdForEngineer && oldTeamIdForEngineer !== targetTeam.teamId) {
            const oldTeamObject = (this.systemData.teams || []).find(
              (t) => t.teamId === oldTeamIdForEngineer
            );
            if (oldTeamObject && Array.isArray(oldTeamObject.engineers)) {
              oldTeamObject.engineers = oldTeamObject.engineers.filter(
                (name) => name !== movedEngineerName
              );
            }
          }
      ...
      addNewCallback: async (newEngineerNameInput) => {
        ...
        if (!newEngineerNameInput || newEngineerNameInput.trim() === '') {
          notificationManager.showToast('Engineer name cannot be empty.', 'warning');
          return null;
        }
        const name = newEngineerNameInput.trim();
        if (
          (this.systemData.allKnownEngineers || []).some(
        ...
        const levelStr = await notificationManager.prompt(
          `Enter level (1-7) for "${name}":`,
          '1',
          'Engineer Level'
        );
        ...
        const yearsStr = await notificationManager.prompt(
        ...
        const skillsStr = await notificationManager.prompt(
          `Enter skills for "${name}" (comma-separated):`,
          '',
          'Skills'
        );
        ...
        let isAIInput = await notificationManager.prompt(
          `Is "${name}" an AI Software Engineer? (Yes/No)`,
          'No',
          'AI Engineer?'
        );
        ...
        if (isAISWE) {
          const typeStr = await notificationManager.prompt(
            `Enter AI Agent Type for "${name}":`,
            'General AI',
            'AI Agent Type'
          );
        ...
        return {
          ...
          text: `${newEngineerData.name} (L${newEngineerData.level})${newEngineerData.attributes.isAISWE ? ' [AI]' : ''} - (Unallocated)`,
        };
      },
    }).render();
    return container;
