...
const WorkPackageService = {
  ...
  ensureWorkPackagesForInitiatives(systemData, yearFilter = null) {
    ...
    (systemData.yearlyInitiatives || []).forEach((init) => {
      ...
      const existing = (systemData.workPackages || []).filter(
        (wp) => wp.initiativeId === init.initiativeId
      );
      if (existing.length > 0) {
        ...
        existing.forEach((wp) => {
          ...
          wp.deliveryPhases =
            wp.deliveryPhases ||
            STANDARD_WORK_PACKAGE_PHASES.map((phaseName) => ({
  ...
  recalculateWorkPackageDates(workPackage) {
    if (
      !workPackage ||
      !workPackage.impactedTeamAssignments ||
      workPackage.impactedTeamAssignments.length === 0
    ) {
      return;
    }
  ...
  syncInitiativeTotals(initiativeId, systemData) {
    ...
    const workingDaysPerYear = systemData.capacityConfiguration?.workingDaysPerYear || 261;
    const wpForInit = (systemData.workPackages || []).filter(
    ...
    const { startDate, endDate } = this.getInitiativeDateSpanFromWorkPackages(
      wpForInit,
      initiative
    );
  ...
  syncWorkPackagesFromInitiative(initiative, systemData) {
    ...
    this.ensureWorkPackagesForInitiatives(systemData);
    // const workingDaysPerYear = systemData.capacityConfiguration?.workingDaysPerYear || 261;
    const wps = (systemData.workPackages || []).filter(
      (wp) => wp.initiativeId === initiative.initiativeId
    );
  ...
  applyInitiativeTeamEstimateFromYearPlan(systemData, initiativeId, teamId, sdeYears) {
    if (!systemData || !initiativeId || !teamId) {
      console.error(
        'WorkPackageService.applyInitiativeTeamEstimateFromYearPlan: Missing required params'
      );
      return false;
    }
    ...
    console.log(
      `WorkPackageService: Updated WP ${targetWp.workPackageId} for team ${teamId} (${validatedValue} SDE-years)`
    );
