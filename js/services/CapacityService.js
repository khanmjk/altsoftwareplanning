...
const CapacityService = {
  ...
  calculateTotalStandardLeaveDaysPerSDE(team, globalLeaveTypes, capacityConfig) {
    ...
    globalLeaveTypes.forEach((leaveType) => {
      ...
      const currentGlobalDefaultObj = capacityConfig.leaveTypes.find(
        (lt) => lt.id === leaveType.id
      );
      const globalDefault = currentGlobalDefaultObj
        ? currentGlobalDefaultObj.defaultEstimatedDays || 0
        : 0;
  ...
  calculateTotalVariableLeaveDays(team) {
    ...
    for (const leaveKey in varLeaveImpacts) {
      if (Object.prototype.hasOwnProperty.call(varLeaveImpacts, leaveKey)) {
  ...
  calculateNetCapacity({ team, globalLeaveTypes, capacityConfig, workingDaysPerYear }) {
    const standardLeaveDays = this.calculateTotalStandardLeaveDaysPerSDE(
      team,
      globalLeaveTypes,
      capacityConfig
    );
    ...
    // Per-SDE deductions
    const totalPerSdeDeductions =
      standardLeaveDays + orgEventDays + teamActivityImpacts.daysPerSDE + overheadDays;
  ...
  bulkUpdateTeamCapacity(systemData, options = {}) {
    ...
    const {
      updates = {},
      capacityReductionPercent = null,
      aiProductivityGainPercent = null,
      avgOverheadHoursPerWeekPerSDE = null,
      filter = {},
    } = options;
    ...
    const ensureTeamCapacityAdjustments = (team) => {
      ...
      if (!adj.variableLeaveImpact || typeof adj.variableLeaveImpact !== 'object')
        adj.variableLeaveImpact = {};
      if (!Array.isArray(adj.teamActivities)) adj.teamActivities = [];
      if (
        adj.avgOverheadHoursPerWeekPerSDE === undefined ||
        adj.avgOverheadHoursPerWeekPerSDE === null
      )
        adj.avgOverheadHoursPerWeekPerSDE = 0;
      if (adj.aiProductivityGainPercent === undefined || adj.aiProductivityGainPercent === null)
        adj.aiProductivityGainPercent = 0;
    ...
    const targets = systemData.teams.filter((team) => {
      ...
      if (filter.orgIdentifier) {
        ...
        if (resolvedSdmId && team.sdmId && team.sdmId.toLowerCase() === resolvedSdmId.toLowerCase())
          return true;
    ...
    targets.forEach((team) => {
      ...
      if (aiProductivityGainPercent !== null && aiProductivityGainPercent !== undefined) {
        ...
        if (!appliedFields.includes('aiProductivityGainPercent'))
          appliedFields.push('aiProductivityGainPercent');
      }
      if (avgOverheadHoursPerWeekPerSDE !== null && avgOverheadHoursPerWeekPerSDE !== undefined) {
        ...
        if (!appliedFields.includes('avgOverheadHoursPerWeekPerSDE'))
          appliedFields.push('avgOverheadHoursPerWeekPerSDE');
      }
      if (
        capacityReductionPercent !== null &&
        capacityReductionPercent !== undefined &&
        !isNaN(capacityReductionPercent)
      ) {
        ...
        adj.avgOverheadHoursPerWeekPerSDE =
          (adj.avgOverheadHoursPerWeekPerSDE || 0) + addedOverhead;
        ...
        if (!appliedFields.includes('capacityReductionPercent'))
          appliedFields.push('capacityReductionPercent');
      }
