...
const DEFAULT_SAMPLE_SYSTEM_KEYS = [
  'StreamView',
  'ConnectPro',
  'ShopSphere',
  'InsightAI',
...
const SystemService = {
  ...
  loadSystem(systemId) {
    ...
    if (!systemData && _sampleSystemsData && _sampleSystemsData[systemId]) {
      console.log(
        `SystemService: System '${systemId}' not in storage, loading from default samples.`
      );
  ...
  _getSampleSystemKeys() {
    const sampleIds = Object.entries(_sampleSystemsData).map(
      ([key, data]) => data?.systemName || key
    );
    ...
    if (systemData.yearlyInitiatives) {
      systemData.yearlyInitiatives.forEach((initiative) => {
        ...
        if (!initiative.roi) initiative.roi = JSON.parse(JSON.stringify(defaultROI));
          ...
          for (const key in defaultROI) {
            if (initiative.roi[key] === undefined) initiative.roi[key] = defaultROI[key];
          }
        }
        ...
        if (!initiative.status)
          initiative.status = initiative.isProtected ? 'Committed' : 'Backlog';
  ...
  _augmentCapacityConfiguration(systemData) {
    ...
    if (!systemData.capacityConfiguration) {
      ...
      if (cc.workingDaysPerYear === undefined)
        cc.workingDaysPerYear = defaultCapacityConfig.workingDaysPerYear;
  ...
  _validateEngineerAssignments(systemData) {
    ...
    systemData.teams.forEach((team) => {
      ...
      team.engineers.forEach((engName) => {
        ...
        if (rosterTeamId === undefined) {
          isValid = false;
          errorMessages.push(
            `Engineer '${engName}' in team '${team.teamName}' is not in the Global Engineer Roster.`
          );
        } else if (rosterTeamId !== team.teamId) {
          // Mismatch
          isValid = false;
          errorMessages.push(
            `Engineer '${engName}' is assigned to team '${team.teamName}' but Roster says '${rosterTeamId || 'Unassigned'}'.`
          );
        }
      });
    });
    ...
    if (!isValid) {
      ...
      notificationManager.showToast(
        'Data Inconsistency: Engineer assignments do not match global roster. See console.',
        'error'
      );
    }
  ...
  async deleteCurrentSystem() {
    ...
    if (this.isSampleSystem(systemName)) {
      notificationManager.showToast(
        `Cannot delete built-in sample system: "${systemName}".`,
        'error'
      );
