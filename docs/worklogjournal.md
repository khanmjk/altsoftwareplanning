...
- 14:05:00 SAST - Added lightweight observability for Cloudflare worker dependencies: `GET /api/health` on `smt-blueprints-worker` (D1 schema checks), `GET /health` on `smt-feedback-worker` (config checks), in-app sidebar Cloud status badge (advisory), and a scheduled GitHub Actions synthetic check with README badge.
